...
class Login(object):
    ...
    def update_password(self, username: str, new_password: str, passowrd_repeated: str) -> bool:
        self.conn = sqlite3.connect(self.users_db)
        self.c = self.conn.cursor()
        ...
        if not complete_inputs:
            ...
            self.c.execute(f'UPDATE {self.table} SET password = ? WHERE username = ?', (new_password, username))
            messagebox.showinfo('Password Updated', f'Your new password is {new_password}')
            self.conn.commit()
            self.conn.close()